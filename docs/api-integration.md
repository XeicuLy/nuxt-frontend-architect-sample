...
1. **開発サーバー自動起動**: バックグラウンドでサーバーを起動
2. **起動完了検知**: ヘルスチェックで準備完了を確認
3. **OpenAPIスペック収集**: サーバーから `/api/openapi.yaml` を取得して `public/openapi.yaml` に保存
4. **サーバー自動停止**: 処理完了後にサーバーをクリーンアップ
5. **型定義生成**: ローカルファイルから `shared/types/api/` に型を生成
6. **コード整形**: 生成されたコードを自動整形

**📋 利用可能なコマンド**:

```bash
pnpm generate-types         # 完全自動化（推奨）
pnpm generate-types:manual  # 従来の手動方式（サーバー別起動が必要）
pnpm generate-types:ci      # CI専用（OpenAPIファイル既存前提）
```
